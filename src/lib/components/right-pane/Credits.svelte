<div class="pb-4 pt-4 flex flex-col space-y-4 divide-y divide-light-border dark:divide-dark-border text-neutral-400 hover:text-black dark:hover:text-white">
	<div class="space-y-1">
		<div class="text-sm text-neutral-400">Source Code</div>
		<div>
			<img src="/brands/svelte.svg" class="h-5" alt="svelte-logo">
		</div>
		<div class="text-sm">
			This website was developed from the ground-up using <a target="_blank" href="https://svelte.dev/" title="Svelte" class="underline">Svelte</a>.
			The source code is available here: <a target="_blank" href="https://github.com/imkiptoo/imkiptoo.github.io" title="GitHub" class="underline">GitHub</a>. Feel free to fork and modify it to suit your needs ;)
		</div>
	</div>
	<div class="space-y-1 pt-4">
		<div class="text-sm text-neutral-400">Credits</div>
		<div class="text-sm">
			Icons from <a target="_blank" href="https://icon-sets.iconify.design/" title="Iconify Design" class="underline">Iconify Design</a>,
			fonts from <a target="_blank" href="https://fonts.google.com/" title="Google Fonts" class="underline">Google Fonts</a>
			and brand images from <a target="_blank" href="https://icons8.de/" title="Icons8" class="underline">Icons8</a>
		</div>
	</div>
</div>